...
public class BigRationalImpl implements Comparable<BigRationalImpl>, Fields.BigRationalElement<BigInteger, BigRationalImpl>
{
    ...
    private static boolean isOne(BigRationalImpl that) {
        return that.num == 1 && that.den == 1;
    }

    private static boolean isZero(BigRationalImpl that) {
        return that.num == 0 && that.den == 1;
    }

    private static boolean isWhole(BigRationalImpl that) {
        return that.den == 1 || (that.den == 0 && that.bigDen.equals(BigInteger.ONE));
    }

    private static int signum(BigRationalImpl that) { 
    ...
    public BigRationalImpl( BigInteger numerator, long denominator )
    {
        this(numerator, BigInteger.valueOf(denominator));
    }

    public BigRationalImpl( long numerator, BigInteger denominator )
    {
        this(BigInteger.valueOf(numerator), denominator);
    }

    public static BigRationalImpl[][] newMatrix(int [][] numerators) {
        BigRationalImpl[][] matrix = new BigRationalImpl[numerators.length][];
    ...
    public static BigRationalImpl[] newArray(int [] numerators) {
        BigRationalImpl[] array = new BigRationalImpl[numerators.length];
        for (int col = 0; col < numerators.length; col++) {
            array[col] = new BigRationalImpl(numerators[col]);
        }
        return array;
    }
    ...
    public BigRationalImpl( String s )
    {
        ...
        case 1:
            ...
            if(tokens[NUM].length() <= definteLong) {
                ...
                BigRationalImpl temp = new BigRationalImpl( new BigInteger( tokens[NUM] ) );
                num = temp.num;
                den = temp.den;
                bigNum = temp.bigNum;
                bigDen = temp.bigDen;
        ...
        case 2:
        {
            ...
            BigRationalImpl temp = null;
            if(tokens[NUM].length() <= definteLong && tokens[DEN].length() <= definteLong) {
                // will obviously fit in a long
                if(tokens[DEN].equals("1") || tokens[NUM].equals("1")) {
                    // no need to reduce
                    temp = new BigRationalImpl( Long.parseLong( tokens[NUM] ), Long.parseLong( tokens[DEN] ), null, null );
                } else {
                    // may need to be reduced
                    temp = new BigRationalImpl( Long.parseLong( tokens[NUM] ), Long.parseLong( tokens[DEN] ) );
                }
            }
            if(temp == null) {
                if(tokens[DEN].equals("1") || tokens[NUM].equals("1")) {
                    // no need to reduce
                    temp = new BigRationalImpl( 0, 0, new BigInteger( tokens[NUM] ), new BigInteger( tokens[DEN] ) );
                } else {
                    // won't know if it can be long until it's validated and reduced by the temp c'tor
                    temp = new BigRationalImpl( new BigInteger( tokens[NUM] ), new BigInteger( tokens[DEN] ) );
                }
            }
            num = temp.num;
            den = temp.den;
     ...
     */
    static boolean canAddInteger(BigRationalImpl n) {
        return( n.den == 1 
     ...
     */
    static boolean canMultiplyInteger(BigRationalImpl n) {
        return( n.den == 1 
    ...
    public final static class Gcd {
        ...
        public static long gcd(long j, long k)
        {
            ...
            j = Math.abs(j);
            k = Math.abs(k);
            return(((j|k) & BigRationalImpl.Gcd.LOOKUP_MASK) == 0L)
                    ...
                    ? (long) 0x00FF & BigRationalImpl.Gcd.LOOKUP_TABLE[(int)j] [(int)k]
                            : calculateGcd(j, k);
        }
     ...
     */
    @Override
    public int compareTo( BigRationalImpl that )
    {
     ...
     */
    public static BigRationalImpl min(BigRationalImpl a, BigRationalImpl b) {
     ...
     * @throws NullPointerException if either parameter is null;    
     */
    public static BigRationalImpl max(BigRationalImpl a, BigRationalImpl b) {
        return a.compareTo(b) == 1 ? a : b;
    }
    ...
    public static BigRationalImpl min(Collection<BigRationalImpl> collection) {
        return collection.isEmpty() ? null : min(collection.toArray(new BigRationalImpl[collection.size()]));
    }
    ...
    public static BigRationalImpl max(Collection<BigRationalImpl> collection) {
        return collection.isEmpty() ? null : max(collection.toArray(new BigRationalImpl[collection.size()]));
    }
     ...
     * @return this + n
     */
    public BigRationalImpl plus( int n )
    {
        return n == 0 
                ? this 
                : canAddInteger
     ...
     */
    @Override
    public BigRationalImpl negate()
    {
        return this.isZero()
                ? ZERO
                        : this.notBig()
                        ? new BigRationalImpl( -num, den, null, null )
                                : new BigRationalImpl( 0, 0, bigNum.negate(), bigDen );
    }
     ...
     * @return this - n
     */
    public BigRationalImpl minus( int n )
    {
        return n == 0 
            ? this 
            : canAddInteger
     ...
     */
    @Override
    public BigRationalImpl reciprocal()
    {
        return this.isOne()
                ? ONE
                        : this.isBig()
                        ? new BigRationalImpl( bigDen, bigNum )
                                : new BigRationalImpl( den, num );
    }
     ...
     */
    public BigRationalImpl dividedBy( long d )
    {
    ...
    static BigDecimal toBigDecimal(BigRationalImpl that) {
