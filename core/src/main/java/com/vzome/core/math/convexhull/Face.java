...
class Face {
    ...
    double area;
    private AlgebraicVector centroid;
    AlgebraicNumber planeOffset;
    int index;
    int numVerts;
    ...
    public AlgebraicVector computeCentroid() {
        Set<AlgebraicVector> vectors = new HashSet<>();
        HalfEdge he = he0;
        do {
            vectors.add(he.head().pnt);
            he = he.next;
        } while (he != he0);
        centroid = AlgebraicVectors.calculateCentroid(vectors);
        return centroid;
    }

//    public void computeNormal(AlgebraicVector normal, double minArea) {
...
//            } while (hedge != he0);
//
//            AlgebraicVector p2 = hedgeMax.head().pnt;
//            AlgebraicVector p1 = hedgeMax.tail().pnt;
...
//    }
    ...
    public AlgebraicVector computeNormal() {
        HalfEdge he1 = he0.next;
        HalfEdge he2 = he1.next;

        AlgebraicVector p0 = he0.head().pnt;
        AlgebraicVector p2 = he1.head().pnt;

        AlgebraicVector d2 = p2.minus(p0);

        normal = p0.getField().origin(3); // 3D

        numVerts = 2;

        while (he2 != he0) {
            AlgebraicVector d1 = d2;
            p2 = he2.head().pnt;
            d2 = p2.minus(p0);
            normal = normal.plus(d1.cross(d2));
            he1 = he2;
            he2 = he2.next;
            numVerts++;
        }
//        area = normal.norm(); // sqrt(x^2 + y^2 + z^2)
//        normal = normal.scale(1 / area); // TODO: is it necessary to return the actual unit vector
        return normal;
    }

    private void computeNormalAndCentroid() throws Failure {
        ...
        if (numv != numVerts) {
            fail("face " + getVertexString() + " numVerts=" + numVerts + " should be " + numv);
        }
    }

    private void computeNormalAndCentroid(double minArea) {
        normal = computeNormal(); // ignoring minArea
        centroid = computeCentroid();
        planeOffset = normal.dot(centroid);
    }
    ...
    public static Face create(Vertex[] vtxArray, int[] indices) throws Failure {
        Face face = new Face();
    ...
    private Face() {
//        normal = new AlgebraicVector();
//        centroid = new AlgebraicVector();
        mark = VISIBLE;
    }
     ...
     */
    public AlgebraicNumber distanceToPlane(AlgebraicVector p) {
        return normal.dot(p).minus(planeOffset);
    }
    ...
    private Face connectHalfEdges(HalfEdge hedgePrev, HalfEdge hedge) throws Failure {
    ...
    private void fail(String msg) throws Failure {
        throw new Failure(msg);
    }
    
    void checkConsistency() throws Failure {
        ...
        if (numVerts < 3) {
            fail("degenerate face: " + getVertexString());
        }
        do {
            HalfEdge hedgeOpp = hedge.getOpposite();
            if (hedgeOpp == null) {
                fail("face " + getVertexString() + ": " + "unreflected half edge " + hedge.getVertexString());
            } else if (hedgeOpp.getOpposite() != hedge) {
                fail("face " + getVertexString() + ": " + "opposite half edge "
            ...
            if (hedgeOpp.head() != hedge.tail() || hedge.head() != hedgeOpp.tail()) {
                fail("face " + getVertexString() + ": " + "half edge "
                        + hedge.getVertexString() + " reflected by " + hedgeOpp.getVertexString());
            }
            Face oppFace = hedgeOpp.face;
            if (oppFace == null) {
                fail("face " + getVertexString() + ": " + "no face on half edge " + hedgeOpp.getVertexString());
            } else if (oppFace.mark == DELETED) {
                fail("face " + getVertexString() + ": " + "opposite face " + oppFace.getVertexString() + " not on hull");
            }
            double d = Math.abs(distanceToPlane(hedge.head().pnt).evaluate());
            if (d > maxd) {
                maxd = d;
            }
            numv++;
    ...
    public int mergeAdjacentFace(HalfEdge hedgeAdj, Face[] discarded) throws Failure {
...
//        // head of hedge1.
//
//        AlgebraicVector p0 = hedge0.tail().pnt;
//        AlgebraicVector p1 = hedge0.head().pnt;
//        AlgebraicVector p2 = hedge1.head().pnt;
//
...
//    }
    ...
    public void triangulate(List<Face> newFaces) throws Failure {
        double minArea = 0;
        HalfEdge hedge;
