...
import { createWorkerStore } from './classic/controllers-solid.js';
import { createWorker, fetchDesign } from '../workerClient/index.js';
import { SceneCanvas } from '../viewer/solid/scenecanvas.jsx';

const queryParams = new URLSearchParams( window.location.search );
const relativeUrl = queryParams.get( 'url' ) || getModelURL( 'vZomeLogo' ); // support for legacy viewer usage (old vZome shares)
...
const Online = () =>
{
  const { getScene, postMessage } = createWorkerStore( worker );

  postMessage( fetchDesign( url || getModelURL( 'vZomeLogo' ), { preview: legacyViewerMode, debug: forDebugger } ) );
  ...
  return (
    <ErrorBoundary fallback={err => err}>
        ...
        about={
          <>
            ...
            <Typography gutterBottom>
              ...
              the <Link target="_blank" rel="noopener" href="https://www.facebook.com/vZome">vZome Online suite</Link>.
      ...
      <SceneCanvas height="100%" width="100%" scene={getScene()} toolActions={{}} trackball={true} />
    </ErrorBoundary>
  );
}
